...
// console.log('taks 4:', listOfNames(artists));
...
function removeArtist(orgArray, number) {
  const copy = orgArray.slice();
  copy.splice(number,1);
  return copy;
}

// console.log('taks 5:',removeArtist(artists,4));
...
function addArtist(artist, name, years, genre, nationality, bio) {
  ...
  // console.log('task 6:',addArtist(artists, 'Michael Adam', '1998', 'Web Design', 'Black', 'Student at BloomTech'));
...
function lotsOfArt(artists) {
  ...
  for(let i = 0; i < artists.length; i++){
    if(artists[i].paintings> 100){
      artistsNameArray.push(artists[i].name)
    }
  }
...
function artistByCountry(artist, nationality){
  const arrayNationality = [];
    ...
    if(artist[i].nationality === nationality){
      arrayNationality.push(artist[i].name)
    }
  }
  return arrayNationality;
}
